...
from .utils import GetKey, mapfunc, rake2ftype_BA
...
class BSSA14_nga(object):
    ...
    def __init__(self):
        ...
        self.faults = ['unspecified', 'strike-slip', 'normal',
                       'reverse', 'U', 'NM', 'SS', 'RV']
        self.Dregions = ['GlobalCATW',
                         'ChinaTurkey',
                         'ItalyJapan'] # considered regions
    ...
    def __call__(self, M, Rjb, Vs30, T, rake, Dregion='GlobalCATW',
                 country='California', Mech=3, Ftype=None, Z10=None,
        ...
        self.Z10 = Z10
        if Z10 != None:
            self.Z10 = Z10 / 1000.  # in km, but the input z10 is in meter
    ...
    def distance_function(self, Tother=None):
        ...
        term = ((c1 + c2 * (self.M - self.Mref)) * np.log(R / self.Rref) +
                c3 * (R - self.Rref) + D_c3 * (R - self.Rref))
    ...
    def Vs30toZ1pt0(self, Vs30):
        ...
        if self.country == 'California':
            return ((-7.15 / 4.) * np.log((Vs30**4 + 570.94**4) /
                                          (1360**4 + 570.94**4)))
        if self.country == 'Japan':
            return ((-5.23 / 2.) * np.log((Vs30**2 + 412.39**2) /
                                          (1360**2 + 412.39**2)))
    ...
    def soil_function(self, Vs30=None, Tother=None):
        ...
        pga4nl = np.exp(self.moment_function(Tother=Tpga) +
                        self.distance_function(Tother=Tpga))
        #print 'PGAr: ', pga4nl

        # 2. compute nonlinear site effect
        f4 = self.Coefs[Ti]['f4']
        f5 = self.Coefs[Ti]['f5']
        f2 = (f4 * (np.exp(f5 * (min([self.Vs30, 760]) - 360)) -
                    np.exp(f5 * (760 - 360))))
    ...
    def compute_std(self):
        ...
        if self.Vs30 >= V2:
            ...
            sigma = (phiMR - D_phi_V * (np.log(V2 * 1.0 / self.Vs30) /
                                        np.log(V2 * 1.0 / V1)))
        else:
            sigma = phiMR - D_phi_V
